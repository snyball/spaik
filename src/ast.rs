...
impl<'a> Excavator<'a> {
    ...
    fn bapp(&self, bt: Builtin, args: PV, src: Source) -> Result<AST2> {
        match bt {
            ...
            Builtin::Quote => self.quote(args.car().map_err(|e| e.src(src.clone()).bop(Builtin::Quote))?, src),
            Builtin::Quasi => self.quasi(args.car().map_err(|e| e.src(src.clone()).bop(Builtin::Quasi))?, src),
