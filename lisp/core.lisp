...
(defun container-type-of (x)
  (if (or (nil? x) (cons? x)) 'list (type-of x)))
  ...
  (let ((m (gensym))
    ...
    `(let ((,v ,src))
       (let ((,it (iter ,v)))
         (let ((,m (next ,it)))
           (when (iter-end? ,m)
             (error 'empty (container-type-of ,v)))
           (loop (let ((,x (next ,it)))
                   (when (iter-end? ,x)
                     (break ,m))
                   (when (,ord ,x ,m)
  ...
  (let ((i (int idx)))
    ...
    (cond
      ((vec? xs) (if (< i (len xs))
                     ...
                     alt))
      ((= xs nil) alt)
      ((cons? xs) (or (loop (when (> 0 (dec! i))
                              (break (car xs)))
                       (when (nil? (set xs (cdr xs)))
                         (break)))
                      alt))
      (true (error 'type-error
                   (container-type-of xs))))))
